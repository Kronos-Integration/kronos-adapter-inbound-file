...
class AdapterInboundFile extends baseStep {
	...
	_parseInboundFileMessage(directory, fileName, message) {
		...
		if (fileAbs) {
			...
			fs.exists(fileAbs, function (exists) {
				if (exists) {
					// ok, trigger the file
					self._createReadStreamMessage(fileAbs, fileRel, fileDir);
				} else {
					// write error log
	 ...
	 */
	_createReadStreamMessage(absFileName, fileName, directory) {
		const self = this;
...
module.exports = function (opts) {
	return new AdapterInboundFile(opts);
};
