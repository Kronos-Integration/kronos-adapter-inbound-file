		...
		inEndpoint.receive = function (message) {
			if (message.payload) {
				if (typeof message.payload === 'string') {
					...
					self._parseInboundFileMessage(undefined, message.payload, message);
				} else if (Array.isArray(message.payload)) {
					// take it as an array of file names
					for (let i = 0; i < message.payload.length; i++) {
						// iterate over the files
						self._parseInboundFileMessage(undefined, message.payload[i], message);
					}
					...
					if (message.payload.files) {
						...
						if (Array.isArray(message.payload.files)) {
							for (let i = 0; i < message.payload.files.length; i++) {
								// iterate over the files
								self._parseInboundFileMessage(directory, message.payload.files[i], message);
							}
						} {
							self.error({
								"message": message,
